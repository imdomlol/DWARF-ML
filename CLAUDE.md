...
> **Planned redesign — `docs/PIVOT.md`.** The 4-camera observation and the
> `[action, camera, x, y]` action are agreed to be replaced by a **per-dwarf entity
> model**: one shared policy, trained across parallel worlds, that observes and
> commands *all* dwarves at once (each as a feature vector; "send toward cave/ore"
> becomes the game's gold-paid arrow). It is **not built yet**, so the camera
> contract described below is still the live one. `docs/PIVOT.md` is the design and
> build plan; the sections in this file marked *(being replaced)* are what it
> supersedes.

## Repo layout caveat (important)

`dwarfs-rl/` is the canonical project root; the repo root holds only a thin index README, the `.gitignore`, and `TODO.md`. Everything that runs lives under `dwarfs-rl/` — the mod, the loader, and `python/` (the env `dwarfs_env.py`, the training entrypoint `train.py`, and the test harnesses). Reference docs live in `dwarfs-rl/docs/` (`SETUP.md`, `PROTOCOL.md`, `MULTIWORLD.md`, `HEADLESS.md`, `PIVOT.md`, `WHATS_NEW.md`); `docs/PROTOCOL.md` is the source of truth for the wire format and `docs/PIVOT.md` is the agreed next-step redesign of the obs/action layer. (Historical note: an earlier `dwarf_mod_env.py` env and a `mod_specs.md` doc were removed during consolidation — `TODO.md` tracks the restructure.)
